...
int main(int argc, char **argv) {
  ...
  nl_error(0, "Start");
  while (!done) {
    int nr = fread(&ibuf[recs_recd][0], BSlogger::rec_size, recs_per_file - recs_recd, stdin);
    if (nr > 0 && ibuf[recs_recd][0] != '\xAA' && ibuf[recs_recd][0] != '\0')
      nl_error(3, "Invalid data from BAT_SPAN");
    recs_recd += nr;
    ...
    if (recs_recd > recs_writ) {
      ...
      nr = fwrite(&ibuf[recs_writ][0], BSlogger::rec_size, recs_recd - recs_writ, fp);
      recs_writ += nr;
      if (recs_writ < recs_recd)
        nl_error(3, "Error %d writing to %s", mlf->path);
